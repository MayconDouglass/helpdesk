  ...
  <div class="card-body">
    ...
    <table id="tableBase" class="table table-bordered table-striped">
      ...
      <tbody>
        ...
        <tr>
          <td class="idDataTabText">{{$statusTicket->id_status}}</td>
          <td>{{$statusTicket->descricao}}</td>
          <td>
            <span @if ($statusTicket->status > 0) class="badge badge-success idDataTabText" @else class="badge badge-danger idDataTabText"
              @endif>{{$statusTicket->status ? "Ativo" : "Inativo"}}</span>
          </td>
          <td class="idDataTabText">
            <button type="button" class="btn btn-info btn-sm fa fa-eye" data-toggle="modal"
              data-target="#VisualizarModal" data-codigo="{{$statusTicket->id_status}}"
              data-nome="{{$statusTicket->descricao}}" data-status="{{$statusTicket->status}}">
            </button>
            @foreach ($acessoCargo as $acesso)
            @if (($acesso->role == 2)&&($acesso->status == 1))
            <button type="button" class="btn btn-info btn-sm fa fa-pencil-square-o" data-toggle="modal"
              data-target="#AlterarModal" data-codigo="{{$statusTicket->id_status}}"
              data-nome="{{$statusTicket->descricao}}" data-status="{{$statusTicket->status}}">
            </button>
            @endif
            @endforeach
            ...
            <button type="button" class="btn btn-danger btn-sm fa fa-trash-o" data-toggle="modal"
              data-target="#modal-danger" data-codigo="{{$statusTicket->id_status}}"></button>
            @endif
            @endforeach
          </td>
        </tr>
      ...
      <div class="modal-content">
          <div class="info_modalHeader">
              <div class="modal-header">
                  <h5 class="modal-title" id="CadastroModalLabel">Novo Status Ticket</h5>
          ...
          <div class="modal-body">
              <!-- Form de cadastro -->
          <form class="form-horizontal" method="POST" action="{{action('StatusController@store')}}">
                  @csrf
...
<div class="modal fade" id="AlterarModal" tabindex="-1" role="dialog" aria-labelledby="AlterarModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
      <div class="modal-content">
          <div class="info_modalHeader">
              <div class="modal-header">
                  <h5 class="modal-title" id="AlterarModalLabel">Alterar Status</h5>
                  <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                      <span aria-hidden="true">&times;</span>
                  </button>
              </div>
          ...
          <div class="modal-body">
              <!-- Form de cadastro -->
          <form class="form-horizontal" method="POST" action="{{action('StatusController@update')}}" enctype="multipart/form-data">
                  @csrf
                  <div class="form-group row">
                   
                    <div class="col-sm-12">
                      <input class="form-control" type="hidden" name="idStatusAlt" id="idStatusAlt" required>
                      <label class="control-label">Descrição</label>
          ...
          <form class="form-horizontal">
                  ...
                  <div class="form-group row">
                    ...
                    <div class="col-sm-3">
                      ...
                      <input class="form-control" type="text" name="idStatusView" id="idStatusView" required disabled>
                    </div>
...
@section('js')
<script src="{{url('/')}}/js/pages/status.js"></script>
